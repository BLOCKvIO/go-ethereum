...
func (p *PermissionCtrl) asyncStart() {
	...
	//p.node.Lifecycle(ethereum)
	p.node.GetLifecycle(func(lf node.Lifecycle) bool {
		if r, ok := lf.(*eth.Ethereum); ok {
			ethereum = r
			return true
		}
		return false
	...
	go func(_wg *sync.WaitGroup) {
		...
		pollingTicker := time.NewTicker(1000 * time.Millisecond)
		...
		for {
			...
			case <-pollingTicker.C:
				...
				if !ethereum.Downloader().Synchronising() {
					return
				}
			case <-stopChan:
				return
	...
	log.Debug("permission service: afterStart done")
}
...
func (p *PermissionCtrl) populateStaticNodesToContract() error {
	...
	for _, node := range nodes {
		url := pcore.GetNodeUrl(node.ID().String(), node.IP().String(), uint16(node.TCP()), uint16(0), p.isRaft)
		_, err := p.contract.AddAdminNode(url)
		if err != nil {
			log.Warn("Failed to propose node", "err", err, "enode", node.ID().String())
			return err
		}
