...
contract PermissionsImplementation {
    ...
    string private deployOrg;
    string private deployRole;
    string private orgDeployRole;
    bool private deployEnabled;
    ...
    function setDeployPolicy(
        string calldata _nwDeployOrg,
        string calldata _nwDeployRole,
        string calldata _oDeployRole,
        bool _enabled
    ) external onlyInterface {
        deployOrg = _nwDeployOrg;
        deployRole = _nwDeployRole;
        orgDeployRole = _oDeployRole;
        deployEnabled = _enabled;
    }
    ...
    /** @notice called at the time of network initialization. sets up
      ...
      */
    function init(uint256 _breadth, uint256 _depth)
        external
        onlyInterface
    ...
    /** @notice as a part of network initialization add all accounts which are
      ...
      * @param _account - account id
      */
    function addAdminAccount(address _account)
        external
        onlyInterface
        networkBootStatus(false)
    {
    ...
    function addOrg(
        ...
        voterManager.addVotingItem(adminOrg, _orgId, "", _account, 1);
    ...
    function approveAdminRole(
        ...
        if ((processVote(adminOrg, _caller, 4))) {
            (bool ret, address account) = accountManager.removeExistingAdmin(
                _orgId
            );
            if (ret) {
    ...
    /** @notice function to fetch detail of any pending approval activities
      ...
      * @param _orgId unique id of the organization to which the account belongs
      */
    function getPendingOp(string calldata _orgId)
        external
        view
        returns (
            string memory,
     ...
     * @return true/false
     */
    function isOrgAdmin(address _account, string memory _orgId)
        public
        view
        returns (bool)
    {
     ...
     * @return true/false
     */
    function validateAccount(address _account, string memory _orgId)
        public
        view
        returns (bool)
    {
     ...
     * @return true/false
     */
    function _checkOrgExists(string memory _orgId)
        internal
        view
        returns (bool)
    {
     ...
     * @return true/false
     */
    function checkOrgApproved(string memory _orgId)
        internal
        view
        returns (bool)
    {
     ...
     * @return true/false
     */
    function _checkOrgStatus(string memory _orgId, uint256 _status)
        internal
        view
        returns (bool)
    {
     ...
     * @return true/false
     */
    function _checkOrgAdminExists(string memory _orgId)
        internal
        view
        returns (bool)
    {
     ...
     * @return true/false
     */
    function _roleExists(string memory _roleId, string memory _orgId)
        internal
        view
        returns (bool)
    {
     ...
     * @return true/false
     */
    function _isVoterRole(string memory _roleId, string memory _orgId)
        internal
        view
        returns (bool)
    {
     ...
     */
    function _getUltimateParent(string memory _orgId)
        internal
        view
    ...
    function transactionAllowed(
        ...
        if (typeOfxn == 2 && deployEnabled) {
            return
                roleManager.transactionAllowed(
                    deployRole,
                    deployOrg,
                    _getUltimateParent(deployOrg),
                    typeOfxn
                );
        }
