...
import (
	...
	implbind "github.com/ethereum/go-ethereum/permission/impl/bind"
...
func setup() {
	...
	contrBackend = backends.NewSimulatedBackend(ethConf.Genesis.Alloc, ethConf.Genesis.GasLimit)

	var permUpgrInstanceE *implbind.PermUpgr

	guardianTransactor, _ := bind.NewKeyedTransactorWithChainID(guardianKey, ethereum.BlockChain().Config().ChainID)

	permUpgrAddress, _, permUpgrInstanceE, err = implbind.DeployPermUpgr(guardianTransactor, contrBackend, guardianAddress)
	if err != nil {
		t.Fatal(err)
	}
	permInterfaceAddress, _, _, err = implbind.DeployPermInterface(guardianTransactor, contrBackend, permUpgrAddress)
	if err != nil {
		t.Fatal(err)
	}
	nodeManagerAddress, _, _, err = implbind.DeployNodeManager(guardianTransactor, contrBackend, permUpgrAddress)
	if err != nil {
		t.Fatal(err)
	}
	roleManagerAddress, _, _, err = implbind.DeployRoleManager(guardianTransactor, contrBackend, permUpgrAddress)
	if err != nil {
		t.Fatal(err)
	}
	accountManagerAddress, _, _, err = implbind.DeployAcctManager(guardianTransactor, contrBackend, permUpgrAddress)
	if err != nil {
		t.Fatal(err)
	}
	orgManagerAddress, _, _, err = implbind.DeployOrgManager(guardianTransactor, contrBackend, permUpgrAddress)
	if err != nil {
		t.Fatal(err)
	}
	voterManagerAddress, _, _, err = implbind.DeployVoterManager(guardianTransactor, contrBackend, permUpgrAddress)
	if err != nil {
		t.Fatal(err)
	}
	permImplAddress, _, _, err = implbind.DeployPermImpl(guardianTransactor, contrBackend, permUpgrAddress, orgManagerAddress, roleManagerAddress, accountManagerAddress, voterManagerAddress, nodeManagerAddress)
	if err != nil {
		t.Fatal(err)
	}
	// call init
...
func TestPermissionCtrl_AfterStart(t *testing.T) {
	...
	var contract *impl.Init
	contract, _ = testObject.contract.(*impl.Init)
...
func TestQuorumControlsAPI_OrgAPIs(t *testing.T) {
	testObject := typicalQuorumControlsAPI(t)
	from := getArbitraryAccount()
	invalidTxa := ethapi.TransactionArgs{From: &from}

	// test AddOrg
	orgAdminKey, _ := crypto.GenerateKey()
	orgAdminAddress := crypto.PubkeyToAddress(orgAdminKey.PublicKey)

	txa := ethapi.TransactionArgs{From: &guardianAddress}
...
func testTransactionAllowed(t *testing.T, q *QuorumControlsAPI, txa ethapi.TransactionArgs, expected bool) {
...
func TestQuorumControlsAPI_TransactionAllowed(t *testing.T) {
	...
	if testObject.permCtrl.IsV2Permission() {
		...
		transactionTxa := ethapi.TransactionArgs{From: &acct, To: &guardianAddress, Value: &value}
		contractCallTxa := ethapi.TransactionArgs{From: &acct, To: &guardianAddress, Data: &payload}
		contractCreateTxa := ethapi.TransactionArgs{From: &acct, To: &common.Address{}, Data: &payload}
...
func TestQuorumControlsAPI_RoleAndAccountsAPIs(t *testing.T) {
	...
	invalidTxa := ethapi.TransactionArgs{From: &from}
	acct := getArbitraryAccount()
	txa := ethapi.TransactionArgs{From: &guardianAddress, To: &acct}
	...
	assert.Equal(t, err, errors.New("Invalid account id"))
	testTransactionAllowed(t, testObject, ethapi.TransactionArgs{From: &acct, To: &acct}, false)
	...
	testTransactionAllowed(t, testObject, ethapi.TransactionArgs{From: &acct, To: &acct}, true)
	...
	AccountArray[0] = common.HexToAddress("0fbdc686b912d7722dc86510934589e0aaf3b55a")
	AccountArray[1] = common.HexToAddress("9186eb3d20cbd1f5f992a950d808c4495153abd5")
	AccountArray[2] = common.HexToAddress("0638e1574728b6d862dd5d3a3e0942c3be47d996")
	AccountArray[3] = common.HexToAddress("ae9bc6cd5145e67fbd1887a5145271fd182f0ee7")
...
func typicalPermissionCtrl(t *testing.T, v2Flag bool) *PermissionCtrl {
	...
	testObject.contract = NewPermissionContractService(testObject.ethClnt, testObject.key, testObject.permConfig, false, false, ethereum.BlockChain().Config().ChainID)

	b := testObject.backend.(*impl.Backend)
	b.Contr = testObject.contract.(*impl.Init)
...
func TestParsePermissionConfig(t *testing.T) {
	...
	_ = os.Remove(fileName)
	tmpPermCofig.Accounts = append(tmpPermCofig.Accounts, common.HexToAddress("0xed9d02e382b34818e88b88a309c7fe71e65f419d"))
	blob, _ = json.Marshal(tmpPermCofig)
	if err := ioutil.WriteFile(fileName, blob, 0644); err != nil {
		t.Fatal("Error writing new Node info to file", "fileName", fileName, "err", err)
	}
	...
	_ = os.Remove(fileName)
	tmpPermCofig.InterfAddress = common.HexToAddress("0xed9d02e382b34818e88b88a309c7fe71e65f419d")
	blob, _ = json.Marshal(tmpPermCofig)
	if err := ioutil.WriteFile(fileName, blob, 0644); err != nil {
		t.Fatal("Error writing new Node info to file", "fileName", fileName, "err", err)
	}
